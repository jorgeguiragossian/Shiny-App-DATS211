...
function(input, output, session){
  ...
  output$head2 <- renderText(
    paste("5 states with low rate of", input$var2, "Heart Disease")
  )
  ...
  output$bar <- renderPlotly({
    my_data %>%
      ...
      layout(title = paste("Statewise mortality for", input$var2),
             xaxis = list(title = "State"),
             yaxis = list(title = paste(input$var2, "Mortality from heart disease") ))
  })
  ...
  output$map_plot <- renderPlot({
    new_join %>%
      ggplot(aes(x=long, y=lat,fill=get(input$mortalitytype) , group = group)) +
      geom_polygon(color="black", size=0.4) +
      scale_fill_gradient(low="#73A5C6", high="#001B3A", name = paste(input$mortalitytype, "Arrest rate")) +
      theme_void() +
      labs(title = paste("Choropleth map of", input$mortalitytype , " 2020 mortality rates by state.")) +
      theme(
