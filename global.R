...
library(readxl) # To be able to read excel files


# #### Dataset Manipulation ####
my_data <- read_excel("Causes of Death.xlsx")

#   #mutate(State = states)
# 
# ## create a states object from rownames
# states = rownames(read.csv("Causes of Death.csv"))
# 
...
# my_data1 = my_data %>%
#   #mutate(State = tolower(State))  # converting the state names from my_data dataset to lower case so we can later merge the maps data to our dataset
# 
# 
# ## Add the latitude, longitude and other info needed to draw the ploygon for the state map
# # For the state boundaries available - add the my_data info.
